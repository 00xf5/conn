# Deploy WorthyJoin on a VPS (production path)
...
Then open **Admin → Agent package** at `https://worthyjoin.online/admin/` and upload `agent.zip`. Also place `data/agent/WorthyJoin-Setup.exe` on the server (same folder) so hosts can download a one-click Setup.exe.

Host install (from dashboard link): open link → **Download WorthyJoin** → paste enrollment code → Install. The **ConnectAgent** Windows Service is installed (UAC). The service supervisor keeps the interactive capture agent alive across reboot, lock, and crash. If UAC is denied, Startup-folder watchdog is used as fallback.
...
3. Host opens the link → Download WorthyJoin → paste code → Install → PC appears online  
